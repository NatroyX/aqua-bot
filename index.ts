...
interface NotificationPayload {
  type: "bot_submit" | "vote" | "bot_approved" | "bot_rejected";
  botId: string;
  botName: string;
  userId: string;
  username: string;
  ...
  try {
    ...
    if (type === "bot_submit") {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(`A new bot has been submitted for review`)
        .addFields(
          { name: "Bot Name", value: botName || "Unknown", inline: true },
          { name: "Bot ID", value: botId || "Unknown", inline: true },
          {
            name: "Submitted By",
      ...
      await sendLogMessage(embed, botId, payload);
    } else if (type === "vote") {
      // Vote notification
      const embed = new EmbedBuilder()
        .setTitle("New Vote")
        .addFields(
          ...
          { name: "Bot ID", value: botId || "Unknown", inline: true },
          { name: "Voted By", value: username || "Unknown User", inline: true }
        )
        .setColor("#2ecc71")
        .setTimestamp();

      await sendLogMessage(embed, botId, payload);
    } else if (type === "bot_approved") {
